...
| `onchain-cancel` | `{ commitmentHash, speculationId, contestId, makerSide, txHash, gasPolWei: string }` — `MatchingModule.cancelCommitment` landed (shutdown kill, `cancel-stale --authoritative`, the routine `cancelMode: onchain` cancel of a matched remainder — a visible `partiallyFilled` retained partial OR a recovered soft-cancel, i.e. soft-cancelled then matched on chain — OR the §5.1 own-state-health active cancel-sweep (PR3b-ii) under `cancelMode: onchain`). Record → `authoritativelyInvalidated`. |
...
| `nonce-floor-raise` | Reserved. `raiseMinNonce` bulk-invalidate is documented as a future optimization (the on-chain cancels are currently per-commitment across the shutdown kill path, `cancel-stale --authoritative`, the routine `cancelMode: onchain` partial-remainder cancel, and the §5.1 own-state-health active cancel-sweep). |
