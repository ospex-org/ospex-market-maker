...
| `cancel-stale [--authoritative] [--keystore <p>] [--ignore-missing-state] [--json]` | config, state | state (lifecycle stamps), telemetry log, **off-chain DELETE; on-chain if `--authoritative` (per-record `cancelCommitment`, or one `raiseMinNonce` per speculation under `orders.onchainCancelStrategy: bulk-nonce`)** | `{schemaVersion:1, cancelStale: CancelStaleReport}` | clean cleanup | any `errored > 0`, `gasDenied > 0`, or `blockedMissingPayload > 0` (incomplete sweep); refusal (`mode.dryRun:true`, no keystore, state-loss without override, dry: synthetic hash in state, ...) |
...
```json
{ "ts": "<ISO-8601>", "runId": "<runId>", "maker": "0x…", "kind": "<TelemetryKind>", ...payload }
```
...
`maker` (the instance's lowercased wallet address) is present on every **live**-run line and **absent in dry-run** (no signer). It is a **reserved key** — alongside `ts` / `runId` / `kind`, a payload may not set it — so a shared `telemetry.logDir` is attributable per instance, and the boot-time state-loss fail-safe scopes its "a prior run happened" check to this maker (a sibling instance's logs no longer false-trip the hold). Pre-this-release lines may lack it.
...
| `kind` | Payload |
|---|---|
| `candidate` | `{ contestId?: string, skipReason?: CandidateSkipReason, takerSide?: 'away' \| 'home', market?: 'spread' \| 'total', ...skip-specific }`. **No `skipReason`** = the contest was *tracked* (and `contestId` is present). `market` is present on a **per-market** event — the discovery confirm-loop refusals + the success (tracked) candidate, and every `reconcileMarket` gate — to attribute it to that market when a contest carries several; it is **absent for moneyline** (the unmarked default) and on the contest-level newcomer skips (`no-open-speculation` / `start-too-soon` / `tracking-cap-reached` / `no-reference-odds` emitted before `getContest`, which span all wanted markets and so are not market-specific — each of these reasons ALSO has a per-market form, carrying `market`, from the post-`getContest` confirm loop / `reconcileMarket`). Skip-specific fields: `cap-hit` → `contestId`, `takerSide`; `tracking-cap-reached` / `no-reference-odds` / `no-open-speculation` / `reference-line-mismatch` / `stale-reference` / `start-too-soon` / `refused-pricing` / `untracked` → `contestId` plus skip-reason-specific context (`untracked` = a tracked contest that left the discovery listing and is being DRAINED — its visible quotes are pulled each tick, never re-quoted, until the pull succeeds and a later discovery cycle untracks it; `reference-line-mismatch` = spread/total only — the tracked speculation's on-chain line diverged from the line the reference odds are priced for, so its visible quotes are pulled and it refuses *before pricing* rather than post the reference price at a different line; the discovery refresh follows the oracle line — re-binding to the open spec at the new line, debounced by `orders.replaceOnLineMoveTicks` — so this is a *residual* refusal (a sub-threshold move debounced, or no open spec at the oracle line yet)); `fee-budget-exhausted` → `contestId`, `market?` (a seed's *before-posting* refusal, emitted per side: a seed reaches the per-side submit gate, where `canSpendFee` denies the per-day creation-fee budget `risk.maxDailyFeeUSDC` (default `0` = seed posting disabled — every seed refuses here). A seed posts only when the budget is positive — the second seeding opt-in alongside `marketSelection.seedSpeculations`); `gas-budget-blocks-settlement` → `purpose: 'settleSpeculation' \| 'claimPosition'`, `speculationId`, `contestId`, `makerSide`, `mayUseReserve: boolean`, `todayGasSpentPolWei`, `maxDailyGasPolWei`, `emergencyReservePolWei`, `detail`; `gas-budget-blocks-onchain-cancel` → `commitmentHash`, `speculationId`, `makerSide`, `contestId?`, `todayGasSpentPolWei`, `maxDailyGasPolWei`, `emergencyReservePolWei`, `detail`. **Producers — two emit shapes:** (1) **automatic & reserve-preserving** — `canSpendGas(mayUseReserve: false)`, **includes `contestId`** (all route through the shared `onchainCancelCommitment`): the routine `orders.cancelMode: onchain` cancels (a retained `partiallyFilled` remainder OR a recovered soft-cancel), the funding guard's `underfundedCancelMode: onchain` sweep, and the §5.1 own-state-health active cancel-sweep (PR3b-ii) — an automatic guard must not burn the emergency reserve, so each leaves the record matchable and retries (on the normal cadence, or the next held tick after a gas-denied break). (2) **operator-explicit & reserve-eligible** — `mayUseReserve: true`, **omit `contestId`**: the shutdown kill / `cancel-stale --authoritative` paths, which break the cancel loop on the first denial since today's spend only grows; `gas-budget-blocks-reapproval` → `purpose: 'positionModule-approve' \| 'treasuryModule-approve'`, `todayGasSpentPolWei`, `maxDailyGasPolWei`, `emergencyReservePolWei`, `detail` (no `contestId` — the boot-time auto-approve isn't per-market; the `treasuryModule-approve` purpose is the seeding `TreasuryModule` allowance approve, gated on the seeding double opt-in); `already-settled` → `purpose: 'settleSpeculation'`, `speculationId`, `contestId`, `makerSide`, `outcome: 'alreadySettled' \| 'recovered'`, `winSide`, `revertedTxHash?`, `gasPolWei?`, `gasAccountingGap?`; `already-claimed` → `purpose: 'claimPosition'`, `speculationId`, `contestId`, `makerSide`, `outcome: 'alreadyClaimed' \| 'recovered'`, `revertedTxHash?`, `gasPolWei?`, `gasAccountingGap?` — the two idempotent-skip reasons emit **no** `settle`/`claim` event and (for `already-claimed`) **no** payout; a reverted inclusion-race tx of ours debits `gasPolWei` (summed under `settle`/`claim` respectively), else `gasAccountingGap: true`. |
| `quote-intent` | `{ contestId, speculationId, sport, awayTeam, homeTeam, canQuote: boolean, away: QuoteSideSummary \| null, home: QuoteSideSummary \| null, notes: string[], skew?: { signal, inventoryUSDC } }`. `QuoteSideSummary` = `{ takerOddsTick, takerImpliedProb, makerSide, makerOddsTick, positionType, sizeUSDC, sizeWei6 }`. `skew` (inventory-aware price lean, DESIGN §5) is present **only when `pricing.inventorySkew.enabled`**: `signal` ∈ [-1, 1] is the *requested* lean (away leans up / home down by `signal × halfSpread`) — the runner clamps the realized move to the protocol tick band, so an extreme line can realize a smaller lean than `signal × halfSpread` (the clamp only reduces skew). `inventoryUSDC` is the signed per-speculation HELD-position imbalance that drove it. Omitted when skew is off, so a skew-disabled run's NDJSON is byte-identical (additive, no `schemaVersion` bump). |
| `risk-verdict` | `{ contestId, speculationId, sport, awayTeam, homeTeam, allowed: boolean, awayOffer: { allowed: boolean, sizeUSDC: number, headroomUSDC: number }, homeOffer: {…}, notes: string[] }`. Emitted **after** pre-engine gates pass (not for `no-reference-odds` / `start-too-soon` / `stale-reference` / `no-open-speculation` / `reference-line-mismatch` skips). A SEED reaches the engine (it prices) and so DOES emit `risk-verdict`; its `fee-budget-exhausted` refusal is a per-side *submit*-gate skip, after the engine ran. |
...
| `onchain-cancel` | `{ commitmentHash, speculationId, contestId, makerSide, txHash, gasPolWei: string }` — `MatchingModule.cancelCommitment` landed (the routine `cancelMode: onchain` cancel of a matched remainder — a visible `partiallyFilled` retained partial OR a recovered soft-cancel, i.e. soft-cancelled then matched on chain — the funding guard's `underfundedCancelMode: onchain` sweep, the §5.1 own-state-health active cancel-sweep (PR3b-ii), or the operator-explicit shutdown kill / `cancel-stale --authoritative`). Record → `authoritativelyInvalidated`. **Under `orders.onchainCancelStrategy: bulk-nonce`** the four authoritative *sweep* paths (funding-guard, §5.1 stream-health, shutdown kill, `cancel-stale --authoritative`) emit `nonce-floor-raise` (one `raiseMinNonce` per speculation) instead of this per-record event — see the `nonce-floor-raise` row; the routine `cancelMode: onchain` partial-remainder / recovered-soft-cancel cancel is always per-record and stays `onchain-cancel`. |
...
| `approval` | `{ purpose: 'positionModule' \| 'treasuryModule', spender, currentAllowance, requiredAggregateAllowance, amountSetTo, walletBalanceWei6?, txHash, gasPolWei }` — `walletBalanceWei6` present in `mode:'exact'`, absent in `mode:'unlimited'`. `purpose: 'treasuryModule'` is the boot-time seeding allowance approve (the allowance the maker creation-fee share is pulled from at a seed's first match); raise-only, target = the wallet USDC balance (wallet-bounded), gated on the seeding double opt-in (`seedSpeculations` + `maxDailyFeeUSDC > 0`). **Self-heal variants** (the boot approve's first attempt threw and {@link healApprovalOnce} recovered) carry an extra discriminator and may OMIT some fields: `healed: 're-approved'` — a bounded re-approve landed; full fields (`amountSetTo`, `txHash`, `gasPolWei`). `healed: 'landed'` — the first tx actually confirmed (a post-send step threw); we hold its receipt, so `txHash` + `gasPolWei` are present but there is **no `amountSetTo`** (no re-approve was sent). `gasAccountingGap: true` — a lost-but-landed approve whose gas can't be billed (no receipt in hand): **no `amountSetTo` and no `gasPolWei`**, and `txHash` is present only for the UNKNOWN-reconcile case. Consumers must treat `amountSetTo` / `gasPolWei` as optional and read `healed` / `gasAccountingGap` to classify. |

### 3.4 Fills + positions

| `kind` | Payload |
|---|---|
| `fill` (source `'own-state-stream'`) | `{ source: 'own-state-stream', commitmentHash, speculationId, contestId, sport, awayTeam, homeTeam, takerSide, makerSide, positionType, makerOddsTick, newFillWei6, cumulativeRiskWei6, market?: 'spread' \| 'total', feeUsdcWei6?: string }` — the **only live fill source** since the own-state polling retirement: an owner `fill` delivered over the own-state SSE stream (the canonical state writer in live mode), deduped on `(txHash, logIndex)`. Creates/extends the maker-side position record; `cumulativeRiskWei6` is the position's post-fill own risk. The commitment's `filledRiskWei6` bump arrives separately via the stream's `commitment` event. `market` (omitted for moneyline; shared `FillEventPayload` field across all sources) is the originating commitment's market — `summary` buckets fills/P&L by it. `feeUsdcWei6?` (also a shared `FillEventPayload` field) is the maker's **estimated** protocol creation-fee share, present **only** on the first matched fill of one of the MM's own seed legs and omitted otherwise; the runner folds it into `dailyCounters.feeUsdcWei6` (before the fill emit) and `summary` sums it into `totalFeeUsdcWei6`. It is a **conservative estimate, NOT a realized-fee record** (the protocol charges the fee only when a fill lazily CREATES the speculation, which the MM can't observe from own-state): exact when the MM is the sole seeder of the line, but it over-states by at most one fee per speculation that another maker raced to create first (the MM's seed then matched into the existing speculation and paid no fee). The reducer attributes it only when the FILLING commitment's hash is bound in `seedFeeBySpecKey` (so a non-seed fill at the same line never trips it), absent (byte-identical line) on every moneyline / existing-speculation fill and any seeding-off run. **Agents/scorecards: treat as an estimate, not exact realized spend.** |
...
| `funding-hold` | `{ state: 'entered' \| 'cleared' \| 'sweep-armed', reason: 'funding-shortfall' \| 'read-failed', fundingWei6?, requiredWei6?, walletUsdcWei6?, positionModuleAllowanceWei6?, shortfallHeldForSeconds? }` — the funding guard (DESIGN §6) tripped, cleared, or armed its cancel sweep. `entered` / `cleared` are emitted ONCE per transition (a sustained hold doesn't spam the log). `reason: 'funding-shortfall'` = the wallet can no longer back its matchable-commitment exposure; `'read-failed'` = the balance/allowance read itself threw (fail-closed). **`entered` halts NEW posting straight away and does not by itself mean anything was cancelled.** The C1b cancel sweep runs only from the `sweep-armed` marker onward: that fires at most once per hold episode, when the shortfall is still present on a later successful read at least `fundingGuard.sweepConfirmSeconds` (default 45) after its first observation — with `shortfallHeldForSeconds` carrying how long that was — or immediately, with `reason: 'read-failed'`, when funding could not be read. Once armed, the sweep pulls every `visibleOpen` quote off the relay (`soft-cancel` reason `'funding'`) and, under `underfundedCancelMode: onchain`, authoritatively invalidates the matchable non-terminal records on chain (`onchain-cancel` per record under the default `orders.onchainCancelStrategy: per-commitment`, or one `raiseMinNonce` per speculation → `nonce-floor-raise` under `bulk-nonce`) — **except under `underfundedCancelMode: none`, where the marker still fires but the sweep is a no-op**, so `sweep-armed` means "the sweep may now run", not "something was cancelled". Correlate with `soft-cancel` / `onchain-cancel` / `nonce-floor-raise` events for what actually happened. No NEW posting until `cleared`. `sweepConfirmSeconds: 0` restores the pre-#160 *timing*, where `entered` and the sweep coincide — the marker itself is emitted whatever the window's length, so a `0` run still carries this third `state` value. The wei6 numeric context is attached when known; `shortfallHeldForSeconds` and the wei6 fields are absent on a `read-failed` arm (that path produced no reading to report). |
...
| `foreign-maker-commitments` | `{ count: number, commitmentHashes: string[] }` — **one-time live-boot diagnostic** for the one-wallet-per-instance rule (MM#6 / DESIGN §12). Emitted once at live boot when the maker wallet has open commitments that THIS instance did NOT post (absent from the pre-stream local state) — a likely sign a SECOND MM is running on the same wallet (mutual quote-cancellation + `unknown-own-fill` cold-restart churn). **WARN / advisory-only — never refuses startup**: a foreign open commitment can also be benign (not-yet-reloaded prior-run residue, or a hand-posted quote), so the WARN says "may be". `count` is the full foreign count; `commitmentHashes` is capped at the first 20, so `commitmentHashes.length` may be `< count`. Best-effort — a `listOpenCommitments` read failure is swallowed (the dangerous case also surfaces later as `unknown-own-fill`). **Live only** — dry-run no-ops (no real wallet / on-chain commitments). |
| `position-allowance-short` | `{ currentAllowanceWei6: string, requiredWei6: string }` — **one-time live-boot diagnostic**. Emitted once at live boot when `approvals.autoApprove: false` AND the maker wallet's PositionModule USDC allowance is below the configured required allowance (`requiredPositionModuleAllowanceUSDC(risk)`, `floor`-rounded to wei6 to match `ospex-mm doctor`'s `allowance` check). With auto-approve off the MM never raises the allowance, so if the operator hasn't approved manually either, every posted commitment's FIRST match would revert at the USDC pull. **WARN / advisory-only — never refuses startup** (also a loud stderr `[runner] WARNING` naming the fix: `ospex approvals setup` / `ospex commitments approve`, or `approvals.autoApprove: true`). Best-effort — a `readApprovals` failure is swallowed (a real RPC outage surfaces via the funding guard and the first match). **SILENT** when the allowance is sufficient, when `autoApprove: true` (the MM raises it itself — see `approval`), and in dry-run (the advisory is live-only, behind the same `!dryRun` + `!holding` boot gate as auto-approve). |
| `approval-heal-failed` | `{ purpose: 'positionModule' \| 'treasuryModule', targetWei6: string, currentAllowanceWei6?: string, detail: string }` — **live-boot diagnostic**. The `approvals.autoApprove: true` boot approve threw and the bounded self-heal ({@link healApprovalOnce}) could NOT confirm the allowance reached target — the allowance re-read/reconcile threw, the UNKNOWN-tx reconcile was **inconclusive** (every on-chain allowance read failed across the window, so a possibly-still-pending tx is NOT re-approved — fail-closed to avoid a double-spend), the one bounded re-approve was gas-blocked or threw, or a post-re-approve confirm read still showed it short. `detail` states which (`UNKNOWN-tx reconcile inconclusive: …` / `allowance re-read failed: …` / `re-approve gas-blocked: …` / `re-approve threw: …` / `allowance still short after one bounded re-approve`); `currentAllowanceWei6` is present when a definitive short read was observed. **LOUD** — also a stderr `[runner] WARNING` naming the fix. Boot still proceeds; if the allowance is genuinely short the first match (PositionModule) / first seed match (TreasuryModule) reverts at the USDC pull. This is the terminal signal after the self-heal exhausts its one bounded retry — a successful heal emits `approval` (`healed`/`gasAccountingGap`) instead. **Live only** (the auto-approve flow is behind the `!dryRun` boot gate). |
...
| `nonce-floor-raise` | The `orders.onchainCancelStrategy: bulk-nonce` path raised the maker's per-`(contest, scorer, line)` nonce floor via one `MatchingModule.raiseMinNonce`, invalidating every commitment of the maker's on that line (BOTH sides) with `nonce < newMinNonce`. Payload `{ contestId, speculationId, scorer, lineTicks, newMinNonce, onchainFloor, invalidatedCount, txHash?, gasPolWei?, alreadyAtFloor? }` — `txHash` / `gasPolWei` present when a tx was sent; `alreadyAtFloor: true` when the targets were already below an existing floor (no tx, no gas). The affected records are stamped `authoritativelyInvalidated` (headroom released), and the gas folds into the summary's `onchainCancel` bucket. **NEVER carries a field literally named `nonce`** (the telemetry writer's `DENIED_SIGNING_KEYS` backstop rejects it) — the floor is `newMinNonce`. Emitted (in place of per-record `onchain-cancel`) by the funding guard's `underfundedCancelMode: onchain` sweep, the §5.1 own-state-health active cancel-sweep, the shutdown kill (`killCancelOnChain: true`), and `cancel-stale --authoritative` when `onchainCancelStrategy: bulk-nonce`. A group falls back to per-record `onchain-cancel` when the raise would catch a keeper or a record lacks a locally-known nonce. The routine `cancelMode: onchain` partial-remainder cancel is always per-commitment (a targeted subset, never "all on the speculation"). |

### 3.7 Skip-reason vocabulary (`CANDIDATE_SKIP_REASONS`)

```
'no-reference-odds' | 'no-open-speculation' | 'reference-line-mismatch'
'stale-reference' | 'start-too-soon'
'cap-hit' | 'refused-pricing' | 'tracking-cap-reached'
'gas-budget-blocks-reapproval' | 'gas-budget-blocks-settlement'
'gas-budget-blocks-onchain-cancel' | 'fee-budget-exhausted'
'partial-remainder-retained' | 'already-settled' | 'already-claimed'
```

`refused-pricing` / `cap-hit` arrive during the per-market reconcile. The three `gas-budget-blocks-*` arrive from the on-chain write paths (boot approve, auto-settle/claim, shutdown kill / cancel-stale, the routine `cancelMode: onchain` partial-remainder cancel, the funding guard's `underfundedCancelMode: onchain` sweep, and the §5.1 own-state-health active cancel-sweep). `partial-remainder-retained` marks a `partiallyFilled` remainder the runner left in place — never off-chain-cancelled (the API rejects a DELETE once matched), never reposted over (would double side exposure); its payload is `{ commitmentHash, contestId, speculationId, makerSide, takerSide, reason }`, where `reason ∈ {side-not-quoted, stale, mispriced, duplicate, shutdown}` is why it would have been actioned were it a `visibleOpen`. `already-settled` is the auto-settle idempotent skip — emitted when `ensureSpeculationSettled` finds the speculation already settled (pre-flight) or recovers from a concurrent settle, so a lost race is a boring skip rather than an `error`. Payload `{ purpose: 'settleSpeculation', speculationId, contestId, makerSide, outcome: 'alreadySettled' | 'recovered', winSide, revertedTxHash?, gasPolWei?, gasAccountingGap? }` — `gasPolWei` is present (and debited to the daily counter) when our settle reverted on inclusion (POL was spent on the lost race); `gasAccountingGap: true` flags that such a reverted tx's gas couldn't be fetched, so budget state isn't exact. `already-claimed` is the auto-claim idempotent skip — emitted when `ensurePositionClaimed` finds the position already claimed (pre-flight) or recovers from a benign already-claimed race (a prior run / concurrent caller / `claimable`-projection lag), so it's a boring skip rather than an `error`, and it is **NOT** a `claim` event (no event-sourced payout — the contract zeroes economic fields post-claim, so none is derived). Payload `{ purpose: 'claimPosition', speculationId, contestId, makerSide, outcome: 'alreadyClaimed' | 'recovered', revertedTxHash?, gasPolWei?, gasAccountingGap? }` — same gas semantics as `already-settled` (a reverted inclusion-race claim of ours debits `gasPolWei`, summed under `claim`; `gasAccountingGap: true` flags an unfetchable reverted receipt). The run summary classifies these positions `alreadyClaimed` (not `wonUnclaimed`) and folds no payout into realized P&L. `reference-line-mismatch` is **spread/total only** — emitted in the per-market reconcile *before pricing* when the tracked speculation's on-chain line (away-perspective `lineTicks`) diverges from the line the reference odds are priced for (`round(awayLine × 10)` for spread, `round(line × 10)` for total); its visible quotes are pulled and it refuses rather than post the reference price at a different line. Moneyline has no line and never hits it. The discovery refresh **follows** the oracle line — re-binding a tracked market to the open speculation at the new line and re-quoting there (first pulling the departing speculation's `visibleOpen` quotes off the book, so a same-tick re-bind — discovery runs before the per-market reconcile, so a re-bind on the first tick the oracle reads the new line never reaches this gate — can't strand the old-line legs on the relay book until expiry; skipped for a seed placeholder source, whose records migrate placeholder→real via own-state instead), debounced by `orders.replaceOnLineMoveTicks` (re-bind only when the implied line moves more than that many ticks from the quoted line; `0` = follow every move) — so a divergence reaching this gate is a *residual* one: a sub-threshold move being debounced, or no open speculation at the oracle line yet. `fee-budget-exhausted` is a **seed** market's *before-posting* refusal, emitted per side. When `marketSelection.seedSpeculations` is on, discovery tracks a seed market at the oracle-primary line where **no on-chain speculation exists yet**, carrying a placeholder `speculationId`. A seed prices like any market (it reaches the engine — so it DOES emit `risk-verdict`) and reaches the per-side submit gate, where `canSpendFee` checks the per-day creation-fee budget (`risk.maxDailyFeeUSDC`, default `0` = seed posting disabled). With a zero budget every priced seed refuses here; with a positive budget the seed POSTs and the protocol lazily creates the speculation on first match — the maker owes a creation-fee share then, marked pending via `seedFeeBySpecKey` and attributed (as a conservative ESTIMATE — see §4.1) once at the fill. Posting a seed therefore requires BOTH opt-ins — `seedSpeculations` AND `maxDailyFeeUSDC > 0`. It is emitted **only** for a seed: a non-seed market whose speculation has vanished surfaces instead as `transient-failure` (the per-speculation read throws) or `no-open-speculation` (a closed spec) — never this. Payload `{ contestId, market? }` (the `market` tag is spread/total; omitted for moneyline). The others are pre-engine market-discovery gates.
...
interface MakerState {
  ...
  seedFeeBySpecKey: Record<string, { feeUsdcWei6: string; charged: boolean; hashes: string[] }>;  // ESTIMATED maker creation fee per SEED speculation, keyed by the placeholder `seed:contestId:marketType:lineTicks`; written by the seed-posting path when a seed POSTs (live + seeding double opt-in on; empty unless a seed has POSTED — seeding off, or seeding on but nothing seeded yet, leaves it empty. NB a moneyline market CAN be seeded (`seed:contestId:moneyline:0`), so this is NOT a "moneyline-only" condition). `hashes` = the MM's own seed-leg commitment hashes at this line; the fill reducer attributes the fee ONLY when the FILLING commitment's hash is one of them, so a later non-seed commitment at the same line can't trip a stale marker (no phantom fee) — and it survives an own-state rebaseline (the hash is server-stable). `charged` flips at the seed's first matched fill. The figure is a CONSERVATIVE ESTIMATE (the on-chain fee is charged only at lazy CREATION, which the MM can't observe from own-state) — exact for a sole seeder, an over-estimate by one fee per speculation another maker raced to create first. The fee gate also counts every OTHER uncharged marker with a still-live leg as committed budget (a conservative per-day cap). Additive: a pre-this-field state file migrates to `{}`, a pre-`hashes` entry to `hashes: []`, a malformed entry loads as `lost`
...
interface LiveMetrics {
  ...
  totalFeeUsdcWei6: string;             // Σ fill.feeUsdcWei6 — ESTIMATED maker seed creation-fee (conservative, may over-state by one fee per speculation another maker raced to create first; NOT folded into realizedPnl); '0' on a seeding-off run, or any run where no seed fill has carried a fee (a moneyline market can be seeded too, so NOT a "moneyline-only" condition)
}
...
| `CandidateSkipReason` (15 values) | [`src/telemetry/index.ts`](./src/telemetry/index.ts) — `CANDIDATE_SKIP_REASONS` |
...
- On-chain authoritative (costs POL gas, mayUseReserve): `ospex-mm cancel-stale --authoritative`. Records → `authoritativelyInvalidated`. Per-commitment (the default): one `MatchingModule.cancelCommitment` per record flips `s_cancelledCommitments[hash]` (`onchain-cancel` event). Under `orders.onchainCancelStrategy: bulk-nonce`: one `MatchingModule.raiseMinNonce` per `(contest, scorer, line)` speculation raises the maker's nonce floor so every sub-floor commitment reverts `NonceTooLow` (`nonce-floor-raise` event) — falling back to per-commitment for a speculation whose floor would catch a keeper or a record with no locally-known nonce.
