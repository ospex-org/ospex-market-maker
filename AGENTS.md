...
| `funding-hold` | `{ state: 'entered' \| 'cleared', reason: 'funding-shortfall' \| 'read-failed', fundingWei6?, requiredWei6?, walletUsdcWei6?, positionModuleAllowanceWei6?, atBlock? }` — the funding guard (DESIGN §6) tripped or cleared. Emitted ONCE per enter/clear transition (a sustained hold doesn't spam the log). `reason: 'funding-shortfall'` = the wallet can no longer back its matchable-commitment exposure; `'read-failed'` = the guard could not obtain a comparison it trusts (fail-closed) — a balance / allowance / filled-risk read threw, the filled-risk snapshot did not cover every selected commitment, or a chain filled amount exceeded a record's own risk. **Both sides of the comparison are chain reads at one block** (#160): `requiredWei6` sums `riskAmount - MatchingModule.s_filledRisk[hash]` over the matchable set, and the balance / allowance reads are pinned to the block the filled-risk read resolved. `atBlock` is that block, as a decimal STRING (a block number can exceed `Number.MAX_SAFE_INTEGER`); it is absent on a `read-failed` transition and on the clear emitted when nothing is matchable — neither resolves a block. While held, the funding sweep pulls every `visibleOpen` quote off the relay (`soft-cancel` reason `'funding'`) and, under `underfundedCancelMode: onchain`, authoritatively invalidates the matchable non-terminal records on chain (`onchain-cancel` per record under the default `orders.onchainCancelStrategy: per-commitment`, or one `raiseMinNonce` per speculation → `nonce-floor-raise` under `bulk-nonce`); no NEW posting until cleared. The wei6 numeric context is attached when known. |
