...
interface MakerState {
  ...
  seedFeeBySpecKey: Record<string, { feeUsdcWei6: string; charged: boolean }>;  // maker creation fee per SEED speculation, keyed by the placeholder `seed:contestId:marketType:lineTicks`; written by the seed-posting path when a seed POSTs (live + seeding double opt-in on; empty on any moneyline-only / seeding-off run); `charged` flips at the fee-incurring first fill. Additive: a pre-this-field state file migrates to `{}`, a malformed entry loads as `lost`
