...
'would-create-lazy-speculation' | 'stale-reference' | 'start-too-soon'
...
interface MakerState {
  ...
  seedFeeBySpecKey: Record<string, { feeUsdcWei6: string; charged: boolean }>;  // maker creation fee per SEED speculation, keyed by the placeholder `seed:contestId:marketType:lineTicks`; written ONLY by the seed-posting path (empty otherwise → dormant); `charged` flips at the fee-incurring first fill. Additive: a pre-this-field state file migrates to `{}`, a malformed entry loads as `lost`
...
interface LiveMetrics {
  ...
  totalFeeUsdcWei6: string;             // Σ fill.feeUsdcWei6 (maker creation-fee shares); the reducer/fold is in place but dormant — stays '0' until a seed marker exists (only the seed-posting path, a later slice, writes one)
}
