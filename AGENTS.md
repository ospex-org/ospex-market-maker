...
| `approval` | `{ purpose: 'positionModule' \| 'treasuryModule', spender, currentAllowance, requiredAggregateAllowance, amountSetTo, walletBalanceWei6?, txHash, gasPolWei }` — `walletBalanceWei6` present in `mode:'exact'`, absent in `mode:'unlimited'`. `purpose: 'treasuryModule'` is the boot-time seeding allowance approve (the allowance the maker creation-fee share is pulled from at a seed's first match); raise-only, target = the wallet USDC balance (wallet-bounded), gated on the seeding double opt-in (`seedSpeculations` + `maxDailyFeeUSDC > 0`) |
