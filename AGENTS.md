...
interface MakerState {
  ...
  seedFeeBySpecKey: Record<string, { feeUsdcWei6: string; charged: boolean; hashes: string[] }>;  // maker creation fee per SEED speculation, keyed by the placeholder `seed:contestId:marketType:lineTicks`; written by the seed-posting path when a seed POSTs (live + seeding double opt-in on; empty on any moneyline-only / seeding-off run). `hashes` = the MM's own seed-leg commitment hashes at this line; the fill reducer charges the fee ONLY when the FILLING commitment's hash is one of them, so a later non-seed commitment at the same line can't trip a stale marker (no phantom fee) — and it survives an own-state rebaseline (the hash is server-stable). `charged` flips at the fee-incurring first fill. The fee gate also counts every OTHER uncharged marker with a still-live leg as committed budget (a conservative per-day cap). Additive: a pre-this-field state file migrates to `{}`, a pre-`hashes` entry to `hashes: []`, a malformed entry loads as `lost`
...
interface LiveMetrics {
  ...
  totalFeeUsdcWei6: string;             // Σ fill.feeUsdcWei6 (maker creation-fee shares charged at seed first-matches); '0' on any moneyline-only / seeding-off run (no seed marker → no fee fill)
}
