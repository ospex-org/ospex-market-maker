...
- **SSE stream-budget guardrail is now per-host-aware.** A single instance opens `odds.maxRealtimeChannels` odds streams **+ exactly one** composite own-state stream (the own-state reserve constant is corrected from 3 to 1 — own-state ships as one composite connection, not three). The boot warning now (a) fires only when *one* instance's own budget (`maxRealtimeChannels + 1`) exceeds the core-api per-IP cap, and (b) always logs a per-host SSE-budget reminder when streaming, because the core-api per-IP cap is per **egress IP/host** — shared across every co-located instance, which this process can't see. The mirrored default cap is raised 10 → 16 to match core-api. New `docs/DESIGN.md` "SSE connection budget" section documents the per-host math (`MAX_STREAM_CONNECTIONS_PER_IP ≥ N·(maxRealtimeChannels+1)` for N instances), the `odds.subscribe: false` polling-fallback lever to shrink the footprint now, and the deferred odds-stream-multiplexing plan.
