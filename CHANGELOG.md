...
- **Own-state: a truncated-snapshot paging failure no longer leaves phantom rows in the canonical book.** When a cold-connect's snapshot is still accumulating and the transport reconnects (`onStatus('reconnecting')` with an in-flight `pendingBaseline`), the partial accumulation is now discarded so the next cold snapshot **replaces** it instead of being merged onto it. Previously the pinned SDK (v0.6.2) recovered a mid-flight REST paging failure by clearing its cursor and cold-restarting while emitting `reconnecting` (not `resync`), and the runner merged the fresh snapshot's first page onto the stale partial — so rows already filled/cancelled by snapshot time could swap in as phantom canonical quotes. Reset is gated on `reconnecting` **only** (never `degraded`, which the server also emits for `positionsTruncated` during a healthy snapshot) and only when a baseline is in flight, so a normal mid-stream resume is untouched. The complete fix is SDK-side (emit `resync` on paging failure); this is the consumer-side stopgap until that lands.
