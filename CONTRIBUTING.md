...
New behaviour gets tests (`vitest`, under `tests/` or co-located `*.test.ts`). The pricing math, the risk engine's latent-exposure accounting, the aggregate-allowance target, and config-schema validation are the load-bearing units — keep their coverage tight.
