...
- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (211 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- `run --live` — the live micro-maker (the real SDK write calls behind submit / replace / soft-cancel, the on-chain cancel / nonce-floor paths, the keystore-derived signer, fill detection, gas budgeting in POL), `cancel-stale`, `status` — Phase 3; `summary` (reads the NDJSON event log and emits the §2.3 metrics — fill rate (live) / quote competitiveness (dry-run), quote age, latent-exposure peak, …). These currently exit with `not yet implemented`.
...
So: there is no live maker yet — `run --live` isn't implemented. `doctor`, `quote --dry-run`, and `run --dry-run` (the shadow loop) work and post nothing. Don't run anything against real funds.
...
`yarn mm <cmd>` runs the built CLI (`node dist/cli/index.js`). `yarn dev <cmd>` runs it via `tsx` without a build, for iteration. To get the `ospex-mm` binary on your PATH: `yarn build && yarn link` (or `npm link`), then `ospex-mm <cmd>`. Today `doctor`, `quote --dry-run`, and `run --dry-run` (the shadow loop — posts nothing) work; `run --live` / `cancel-stale` / `status` / `summary` exit `not yet implemented` — see *Current scaffold status* above.
