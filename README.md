...
- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (276 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- the on-chain authoritative-cancel and `raiseMinNonce` paths (for `cancel-stale --authoritative` and the kill switch's `killCancelOnChain: true`), gas budgeting in POL, auto-`settleSpeculation` / `claimPosition` / `claimAll`, and the periodic `getPositionStatus(maker)` poll + P&L — the remaining Phase-3 slices.
