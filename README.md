...
- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (286 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- the on-chain authoritative-cancel and `raiseMinNonce` paths (for `cancel-stale --authoritative` and the kill switch's `killCancelOnChain: true`), gas budgeting in POL, auto-`settleSpeculation` / `claimPosition` / `claimAll`, position-status transitions (`active` → `pendingSettle` → `claimable` → `claimed`), and P&L computation — the remaining Phase-3 slices.
