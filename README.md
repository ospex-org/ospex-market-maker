...
What works (Phase 1 — strictly read-only — plus the Phase-2 dry-run shadow loop, which posts nothing):

- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (251 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- `run --live` — the live micro-maker (the real SDK write calls behind submit / replace / soft-cancel, the on-chain cancel / nonce-floor paths, the keystore-derived signer, fill detection, gas budgeting in POL), `cancel-stale`, `status` — Phase 3. These currently exit with `not yet implemented`. (`summary`'s live-mode metrics — fill rate, P&L, gas, fees, settlements — also land in Phase 3, once those events exist.)
- the runner's fill-detection step and the order *execution* layer behind each `would-*` plan item — Phase 3. (The dry-run loop's per-market reconcile is wired — see above; it logs `would-*` decisions, it just doesn't execute them.)

So: there is no live maker yet — `run --live` isn't implemented. `doctor`, `quote --dry-run`, `run --dry-run` (the shadow loop), and `summary` work and post nothing. Don't run anything against real funds.
...
yarn mm run --dry-run                # (Phase 2) shadow mode: the full loop, posts nothing — let it run a while
...
`yarn mm <cmd>` runs the built CLI (`node dist/cli/index.js`). `yarn dev <cmd>` runs it via `tsx` without a build, for iteration. To get the `ospex-mm` binary on your PATH: `yarn build && yarn link` (or `npm link`), then `ospex-mm <cmd>`. Today `doctor`, `quote --dry-run`, `run --dry-run` (the shadow loop — posts nothing), and `summary` work; `run --live` / `cancel-stale` / `status` exit `not yet implemented` — see *Current scaffold status* above.
