...
- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (200 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- the runner's fill-detection step and the order *execution* layer (the real SDK write calls behind submit / replace / soft-cancel + the on-chain cancel / nonce-floor paths) — Phase 3. (The dry-run loop's per-market reconcile is wired — see above; it logs `would-*` decisions, it just doesn't execute them.)
