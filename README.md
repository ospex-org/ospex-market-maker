...
- `yarn install && yarn build && yarn typecheck && yarn lint && yarn test` — clean (188 unit tests across the pricing, config, risk, orders, state, telemetry, ospex-adapter, runner, and CLI-command modules).
...
- the runner's per-market reconcile / fill-detection steps (the rest of `src/runners/` past discovery + reference-odds tracking, above) — Phase 2+ — and the order *execution* layer (the SDK write calls behind submit / replace / soft-cancel) — Phase 3.
