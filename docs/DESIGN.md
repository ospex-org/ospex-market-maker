...
- Contests **considered / quoted / skipped** (skips broken out by reason: no-reference-odds, no-open-speculation, reference-line-mismatch, would-create-lazy-speculation, stale-reference, start-too-soon, cap-hit, refused-pricing, tracking-cap-reached, gas-budget-blocks-reapproval, ...).
...
| `pricing/` | Pure functions: vig stripping → fair value; spread derivation (economics or direct mode); two-sided quote-price construction; odds-tick conversion + bounds checks. The shared two-way pricing core is `computeQuote`; non-moneyline markets reuse it through thin per-market adapters in `pricing/markets/` that feed their per-side juice through it and carry the line (`spread.ts` + `total.ts` implemented; moneyline uses `computeQuote` directly). Heavily unit-tested. |
...
Pipeline (`computeQuote` — the shared two-way pricing core, used by every market). Ported in spirit from a prior math sketch, rewritten in TypeScript. Spread / total are priced by thin per-market adapters under `pricing/markets/` that source their per-side juice + line and reuse `computeQuote` verbatim (`spread.ts` + `total.ts` implemented); moneyline uses it directly.
...
### Line-consistency gate — the MM never quotes a mispriced line

A spread / total commitment carries an on-chain line (`lineTicks`); the reference odds are priced *for their own line*. If a tracked speculation's line has diverged from the line the reference odds carry, quoting it would post the reference's price at a different line — a mispriced commitment. So before pricing, the per-market reconcile compares the tracked speculation's `lineTicks` against the line the reference odds imply (`oracleLineTicks` — `round(awayLine × 10)` for spread, `round(line × 10)` for total, matching the protocol's **away-perspective** `lineTicks` convention) and, on a divergence, pulls any visible quotes and refuses (`candidate skipReason: reference-line-mismatch`) rather than misprice. Moneyline has no line (`oracleLineTicks → null`) and is unaffected, so v0 behavior is unchanged.

The discovery refresh **follows** the oracle: each cycle it re-binds a tracked spread / total market to the open speculation at the oracle line and re-quotes there, so the gate above stays satisfied as the line moves. Re-binding is debounced by `orders.replaceOnLineMoveTicks` — the market only chases a move strictly larger than that many away-perspective ticks (10×-scaled, so `5` = half a point) from the line it's quoting; `0` (the default) follows every move. A divergence that still reaches the gate is therefore a *residual* one — a sub-threshold move being debounced, or no open speculation at the oracle line yet — and refuses (`reference-line-mismatch`) until a later refresh resolves it (a move worth chasing with an open speculation to chase, or the oracle returning to the held line). Re-binding to a new line re-keys the tracked market under the new line (the map keys on `marketKey(contest, market, lineTicks)`); a re-quote at the same line just re-points the speculation id.
...
orders:
  ...
  replaceOnLineMoveTicks: 0         # spread/total only — re-bind a tracked market to the open spec at the oracle's
                                    #   new line (and re-quote) when the implied line moves MORE than this many ticks from
                                    #   the quoted line. Ticks are 10x-scaled (half-point = 5, full point = 10) and the
                                    #   compare is strict (N ignores moves of N ticks or fewer). 0 = follow every move (no
                                    #   debounce); larger debounces churn but the line-consistency gate refuses the residual.
