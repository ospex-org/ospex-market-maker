...
orders:
  ...
  cancelMode: offchain              # freeing a partially-filled remainder: "offchain" (default) leaves it to ride to expiry
                                    #   (its off-chain cancel is rejected once matched); "onchain" authoritatively cancels it
                                    #   (POL gas, reserve-preserving) then re-quotes the freed side. See §9. (visibleOpen quotes
                                    #   are always pulled gasless off-chain regardless of this setting.)
