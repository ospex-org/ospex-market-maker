...
- **Partially-filled remainders — the side is occupied.** Once a commitment has *any* match, its off-chain cancel is rejected by the relay (`409 COMMITMENT_MATCHED`). So a `partiallyFilled` remainder is **never** off-chain-cancelled and **never** reposted over — stacking a fresh same-side quote on a live remainder would double the side's matchable exposure. The reconciler treats a live partial remainder as **occupying its maker side**: it leaves the remainder in place (logging a `partial-remainder-retained` candidate with the reason it would otherwise have acted — `side-not-quoted` / `stale` / `mispriced` / `duplicate`), pulls only any redundant `visibleOpen` on that side, and re-opens the side for a fresh quote only once the remainder `expires` (≤ `expirySeconds` under `fixed-seconds`) or is authoritatively cancelled. Its remaining (unfilled) risk stays counted in the latent-exposure bucket throughout (§6). Removing a remainder *before* expiry requires an authoritative on-chain `cancelCommitment` (gas) — today via `cancel-stale --authoritative` or the kill switch's `killCancelOnChain` path. (Under `match-time` expiry a remainder occupies its side until game start; that's the case the opt-in `orders.cancelMode: onchain` path addresses.)
