...
6. **Fill monitoring.** Detect matched commitments / new positions (bounded — `commitments.list({ maker, status: open,partially_filled })` plus by-hash lookup of the tracked hashes that disappeared, plus `positions.status(address)` periodically; an SSE stream is a fast-follow — §10); move filled risk from "open / latent quote" to "filled position" in the inventory; recompute P&L; immediately re-price (or pull) the opposite side of a market after a fill, because the fill changed the exposure.
...
- **Fills.** Every tick: `client.commitments.list({ maker, status: open,partially_filled })` (≤ `maxOpenCommitments` rows); diff against last tick's `visibleOpen` hash set; look up by hash the few that disappeared and reclassify (→ `filled` / `expired`, or — for an effective `cancelled` — `authoritativelyInvalidated` **only** when `storedStatus='cancelled'` or `nonceInvalidated`, else `softCancelled` for a row that is merely book-hidden but still matchable on chain; the effective `cancelled` status alone is ambiguous post-book-visibility-split, so the canonical signals decide). On a fill: move the filled risk from open/latent to a filled position (per the §6 counting rule); recompute P&L (realized = settled positions, unrealized = open positions marked to current fair); **immediately re-price or pull the opposite side** of that market. Log a `fill` event. Fill-detection latency is bounded by `pollIntervalMs` until the SSE fills stream lands (§10). **Soft-cancelled commitments are API-hidden from that list**, so a separate per-tick step (`reconcileSoftCancelledFills`) probes each `softCancelled` record's *authoritative cumulative* fill via `getCommitment(hash)` (the data-layer projects the real cumulative even for hidden rows) and converges the commitment record — `filledRiskWei6` up to that cumulative (clamped to risk, never decreasing); the lifecycle **stays `softCancelled`** while any remainder is unfilled and only → `filled` once fully matched — **derived from the cumulative amount, not the API's effective status** (a hidden row still reports effective `cancelled`). A partial fill does **not** promote it to `partiallyFilled`: a book-hidden row must never enter the visible-commitment set (it would always "disappear" from the list, and the disappeared-hash path would read its effective `cancelled` and wrongly release the latent remainder). The risk engine already counts a `softCancelled` record at its *remaining* (`risk − filled`), so the matched portion leaves latent exposure without a lifecycle promotion. It is **commitment-only, never the position** (the position poll owns aggregate convergence — including for soft-cancelled fills — so touching positions here would double-count) and **fails closed**: a `getCommitment` probe failure keeps the record `softCancelled` and skips reconcile + ageOut that tick — a stale signed payload may have just matched, so a probe failure is never read as "unfilled". (A `/v1/fills` cursor/SSE consumer remains a documented future enhancement for when own-state push lands or soft-cancel volume makes per-record probing costly — not the v0 path.) Under `orders.cancelMode: onchain`, a recovered soft-cancel (now `softCancelled` with a matched remainder) is additionally routed — after `reconcileMarkets` — to an authoritative on-chain `cancelCommitment`, the soft-cancelled analogue of the retained-partial cancel: it stops further matching of the still-latent remainder instead of letting it ride to expiry (gas-gated, reserve-preserving; unmatched and past-expiry soft-cancels are skipped). Under the default `offchain` it rides to expiry.
...
- **File mode `0o600` on POSIX.** `StateStore.flush` chmods the temp file to owner-read/write-only before the rename, so the live state file ends up `0600` on Linux / macOS production. On Windows `chmodSync` only toggles the read-only bit; the operator is responsible for the parent-directory ACL.
- **Telemetry / artifact redaction at the wire boundary.** `EventLog.emit` (the NDJSON writer that feeds the scorecard) fails closed on any payload key matching `signature` / `signedPayload` / `commitment` / `nonce`, recursively at any depth. A careless `...record` spread of a `MakerCommitmentRecord` into a telemetry call throws rather than leaking the bundle. Per-emit-site allow-list projection (`commitmentEventPayload` / `softCancelEventPayload`) is the primary defence; the denied-key check is the backstop.
...
2. **Sequencing the SSE fills/positions runner wiring** — design places it after Phase 3 / before the fishbowl; the SDK `subscribe` methods landed in 0.3.0, so the remaining work is the MM wiring (poll kept as reconciliation). Confirm against the broader roadmap.
