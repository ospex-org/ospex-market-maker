...
- Each tick (live only), before posting, the MM checks `min(wallet USDC, PositionModule allowance)` against the **gross** remaining risk of your still-matchable commitments. If it falls short, it **stops posting new quotes** until you top up — and a `funding-hold` telemetry event records it (with the shortfall amounts and the block they were read at).
- **Both numbers are read from chain at the same block.** How much of each commitment is already matched comes from `MatchingModule` itself, not from the indexer-fed stream, and the balance and allowance reads are pinned to the block that read resolved. Until this changed (issue #160), the two sides could describe moments about a poll cycle apart: your own fill briefly looked like a shortfall, and because the cancel response is a sweep across *every* speculation, one fill could pull your whole book. If you saw a hold whose shortfall was exactly the size of a fill that had just landed, that was this.
...
- It **fails closed**: if any of those reads fails — or comes back not describing the commitments it was asked about — the MM enters the hold rather than risk posting commitments it can't back (`failClosedOnReadError: true`, the default).
- This is an *advisory, time-sensitive* guard, not an escrow — funding can still move between the check and a fill, and the check runs at most once per `fundingGuard.checkIntervalMs`. The real protection against unbacked exposure is keeping your wallet funded (generous allowance headroom over your gross quote capacity is still worth having) and using short expiries (see below).
