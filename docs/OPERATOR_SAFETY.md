...
- **Seeding spends USDC.** Each seed speculation costs the maker a one-time creation-fee share at its first match. `maxDailyFeeUSDC` caps that spend per UTC day; once the budget is exhausted, further seeds refuse (`fee-budget-exhausted`) until the next day. The cap is **conservative** — it counts the fee of seeds you've already posted but that haven't matched yet (each will owe its fee at match), so several seeds posted at once can't collectively blow past the budget. Budget it deliberately.
