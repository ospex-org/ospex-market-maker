...
- **Seeding spends USDC.** Each seed speculation costs the maker a one-time creation-fee share at its first match. `maxDailyFeeUSDC` caps that spend per UTC day; once the budget is exhausted, further seeds refuse (`fee-budget-exhausted`) until the next day. Budget it deliberately.
