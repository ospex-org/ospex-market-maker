...
- Each tick (live only), before posting, the MM checks `min(wallet USDC, PositionModule allowance)` against the **gross** remaining risk of your still-matchable commitments. If it falls short, it **stops posting new quotes** straight away, until you top up — and a `funding-hold` telemetry event records it (with the shortfall amounts).
- **Cancelling what is already on the book waits longer than the posting halt does.** A shortfall can be an artefact of timing rather than a real one: a match pulls your USDC on chain immediately, but the MM only learns the fill from its own-state stream a few seconds later, so in between it compares fill-reduced funding against pre-fill exposure. Halting posting on that is harmless; cancelling on it is not, because the cancel sweep reaches **every** market, not just the one that filled. So the sweep only runs once the shortfall is still there **at least** `fundingGuard.sweepConfirmSeconds` later (default 45 s, `0` disables the wait) — in practice at the next funding re-read after that, so roughly one tick on the shipped defaults. You will see a second `funding-hold` event with `state: 'sweep-armed'` at that point, carrying `shortfallHeldForSeconds` = how long it actually waited. A hold that clears before then cancels nothing. Two carve-outs: if the balance read itself *fails* during the wait, the sweep arms immediately (see the fail-closed bullet below), and a restart begins a fresh wait rather than resuming one. Note the wait does not make the check exact: an own-state stream lagging by more than the window can still produce a false sweep, and generous allowance headroom (3–5× gross exposure) is what actually removes the pressure.
- What happens to the quotes already on the book once the sweep runs is set by `fundingGuard.underfundedCancelMode`: **`offchain`** (default) pulls them off the relay so no *new* fills come in (gasless — but the signed payloads stay matchable on chain until they expire, so the hold persists until then); **`onchain`** also authoritatively cancels them on chain (costs gas, but actually clears the exposure so the hold lifts sooner); **`none`** just halts posting and lets them ride to expiry.
- It **fails closed**: if the balance/allowance read itself fails, the MM enters the hold rather than risk posting commitments it can't back (`failClosedOnReadError: true`, the default) — and that path arms the cancel sweep immediately, with no confirmation wait. Funding the MM cannot read is not something waiting resolves.
...
- **With `killCancelOnChain: false` (the default) this is a *soft* stop** — pulled quotes leave the book immediately, but a taker holding the signed payload can still match them on chain until they expire (~2 min with the recommended config). For a *hard* stop (also authoritatively cancels every non-terminal commitment on chain via `cancelCommitmentOnchain` per record — `MatchingModule.cancelCommitment` flips `s_cancelledCommitments[hash]` so subsequent `matchCommitment` calls revert), set `killCancelOnChain: true`. This costs gas — drawn from the emergency reserve via `canSpendGas` with `mayUseReserve: true`. **`orders.onchainCancelStrategy`** picks HOW: `per-commitment` (default) sends one `cancelCommitment` per record; `bulk-nonce` sends one `raiseMinNonce` per `(contest, scorer, line)` speculation (`MatchingModule.raiseMinNonce` sets a per-maker nonce floor so `matchCommitment` reverts `MatchingModule__NonceTooLow` for every commitment below it — BOTH sides of the line at once), far cheaper when you hold several on a line. `bulk-nonce` automatically falls back to per-commitment for any speculation where the floor would also catch a commitment you're keeping (age-scoped `cancel-stale` can have keepers), or a record with no locally-known nonce — so it never burns a keeper. It's safe for continued posting: the SDK reads the on-chain floor and signs new commitments above it.
