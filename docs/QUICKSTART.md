...
> `run --live` is implemented for the reconcile's write path (submits + off-chain cancels) and fill detection (per-tick diff of `listOpenCommitments(maker)`; `fill` / `expire` events; position records). Gas budgeting in POL, auto-settle / auto-claim, and the on-chain kill path are still landing in later Phase-3 slices — keep an eye on the telemetry log and start tiny.
