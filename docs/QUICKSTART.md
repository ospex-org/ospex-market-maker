...
> `run --live` is implemented for the reconcile's write path (submits + off-chain cancels), fill detection (per-tick diff of `listOpenCommitments(maker)`; `fill` / `expire` events; position records), and the position-status poll (mirrors `active` / `pendingSettle` / `claimable` into local state, emitting `position-transition` events on forward steps). Gas budgeting in POL, auto-settle / auto-claim (stamps the local `claimed` status), and the on-chain kill path are still landing in later Phase-3 slices — keep an eye on the telemetry log and start tiny.
