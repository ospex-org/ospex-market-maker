 ...
 * operator cleanup — off-chain by default, on-chain with the flag (per-commitment, or one raiseMinNonce per speculation under `orders.onchainCancelStrategy: bulk-nonce`)),
 ...
 *     most commands; `candidates` is at 2); everything
...
program
  .command('candidates')
  .description('Read-only operator preflight: the (contest, market) pairs the MM could quote right now (verified contest + open speculation for each configured market + reference odds) plus upcoming games that can be turned into quotable contests. Market-aware — classifies each market in marketSelection.markets (default moneyline). Signer-free — no keystore, no passphrase prompt, no writes. An empty listing is a valid answer (exit 0).')
  .option('-c, --config <path>', 'path to the config YAML', DEFAULT_CONFIG_PATH)
...
program
  ...
  .description('One-shot operator command: pull every still-matchable tracked commitment whose age exceeds orders.staleAfterSeconds (records already past expiry + orders.expiryReleaseGraceSeconds are skipped — dead on chain, so there is nothing matchable to invalidate; the runner ages them out to expired). The off-chain (gasless) DELETE applies to visibleOpen records only — a partiallyFilled remainder canNOT be off-chain-cancelled (the API returns 409 once a commitment has matched), so it is skipped off-chain (counted in offchainSkippedPartial) and only --authoritative kills its remaining capacity on chain. --authoritative authoritatively invalidates every non-terminal record (visibleOpen + softCancelled + partiallyFilled) on chain — one cancelCommitment per record by default, or one raiseMinNonce per (contest, scorer, line) speculation under orders.onchainCancelStrategy:bulk-nonce (which falls back to per-record for a speculation whose floor would catch a keeper, or a record with no locally-known nonce); costs POL gas, drawn from the emergency reserve via mayUseReserve:true. STOP `run --live` FIRST — the JSON state file is not multi-process safe (DESIGN §12); cancel-stale shares run\'s single-process state.dir lock, so it will refuse while a run loop is live. Requires mode.dryRun:false in the config (the two-key principle from `run --live`); reads the keystore passphrase from OSPEX_KEYSTORE_PASSPHRASE, else prompts on a TTY. Exits non-zero if any record failed to cancel (errored > 0), a gas-budget denial blocked the on-chain leg (gasDenied > 0), or a legacy book-hidden record had no recoverable signed payload so the authoritative cancel was blocked (blockedMissingPayload > 0).')
  .option('-c, --config <path>', 'path to the config YAML', DEFAULT_CONFIG_PATH)
  .option('--authoritative', 'also authoritatively invalidate on chain — one cancelCommitment per record (default), or one raiseMinNonce per speculation under orders.onchainCancelStrategy:bulk-nonce — gas-gated with mayUseReserve:true (operator-explicit, like the shutdown kill path). Without this, only the gasless off-chain DELETE runs and the signed payload stays matchable on chain until natural expiry.')
  .option('-k, --keystore <path>', 'path to a v3 keystore — overrides config wallet.keystorePath / OSPEX_KEYSTORE_PATH')
