...
export const TELEMETRY_KINDS = [
  ...
  'funding-hold', //               funding guard (C1a) tripped/cleared/sweep-armed — { state: 'entered'|'cleared'|'sweep-armed', reason, fundingWei6, requiredWei6, walletUsdcWei6, positionModuleAllowanceWei6, shortfallHeldForSeconds? }. 'sweep-armed' fires at most once per hold episode, when a shortfall has persisted `fundingGuard.sweepConfirmSeconds` past its first observation (or immediately on a read failure) and the C1b cancel sweep may now run — see DESIGN §6.
  'fill',
  ...
  'position-allowance-short', //   live boot diagnostic: live + approvals.autoApprove=false AND the PositionModule USDC allowance is below the configured required ceiling (requiredPositionModuleAllowanceUSDC, floor-rounded to match `doctor`). The MM won't raise it (auto-approve off) and the operator hasn't either, so the FIRST match would revert at the USDC pull. WARN-only (also a stderr WARNING). Carries { currentAllowanceWei6, requiredWei6 }.
  'approval-heal-failed', //       boot auto-approve threw and the bounded, shape-aware self-heal (healApprovalOnce) could NOT confirm the allowance reached target — the reconcile/re-read threw, the one re-approve was gas-blocked or threw, or a post-re-approve confirm read was still short (`detail` says which). The allowance may be short and the first match/seed could REVERT at the USDC pull. Loud (also a stderr WARNING); boot proceeds. A successful heal emits `approval` (healed/gasAccountingGap) instead. Carries { purpose: 'positionModule'|'treasuryModule', targetWei6, currentAllowanceWei6?, detail }.
  'error', //                      { class, detail }
  'kill',
] as const;
