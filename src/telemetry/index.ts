...
export const CANDIDATE_SKIP_REASONS = [
  'no-reference-odds',
  'no-open-speculation',
  'would-create-lazy-speculation', //  the per-market reconcile declined to post to a market with NO on-chain speculation at its line. Emitted (a) by default for a tracked market whose speculation has gone (a post there would lazily create a new one), and (b) when `marketSelection.seedSpeculations` is on, for a SEED market — one discovery tracks at the oracle-primary line where no speculation exists yet, carrying a placeholder `speculationId`. In the current build this is a hard refusal in BOTH cases (the MM never posts where no speculation exists); the seeding path that actually posts + lazily creates the speculation (paying the protocol creation fee) ships in a later slice. Carries `contestId` + the `market` tag (spread/total; omitted for moneyline).
