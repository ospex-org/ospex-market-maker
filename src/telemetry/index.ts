...
export const TELEMETRY_KINDS = [
  ...
  'approval-heal-failed', //       boot auto-approve exhausted its one bounded self-heal (a broadcast-but-unconfirmed approve, re-read still short, and the one re-approve failed or came back still short) — the allowance may be short and the first match/seed could REVERT at the USDC pull. Loud (also a stderr WARNING). Carries { purpose: 'positionModule'|'treasuryModule', targetWei6, currentAllowanceWei6?, detail }.
  'error', //                      { class, detail }
  'kill',
] as const;
